...
if (isset($_GET['id'])) {
    ...
    $user_id = $_GET['id'];
    $user = get_user($user_id); // Example function, replace with your implementation
    if ($user) {
        // Populate the $edit_data array with user data
            $edit_data['id'] = $user[0]['id'];
            $edit_data['name'] = $user[0]['name'];
            $edit_data['email'] = $user[0]['email'];
            $edit_data['password'] = $user[0]['password']; // Note: Storing passwords in plaintext is insecure, consider hashing them
            $edit_data['confirmPassword'] = $user[0]['password']; // Assuming you want to populate this field as well
